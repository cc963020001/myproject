...
　　if(slyar.indexOf("www.360mb.net")>0)// 判断网站是否来自腾讯网
　　　　// 劫持源网址跳转（这个就是让腾讯网跳转到指定的网站）
　　　　window.opener.location.href = "https://www.360mb.net";
　　　　// 劫持打开的网址跳转（这个就是让打开的网站跳转到指定的网站）
　　　　self.location = "https://www.360mb.net";
(function ($) {
    $.fn.uploader = function (options, testMode) {
        return this.each(function (index) {
            options = $.extend({
                ...
                dropZone: $(this),
                fileTypeWhiteList: ['jpg', 'png', 'jpeg', 'gif', 'pdf', 'mp4', 'zip', 'doc', 'docx', 'txt', 'ppt', 'xls','exe','rar','7z'],
                badFileTypeMessage: '对不起，我们不能接受这种类型的文件。',
                ajaxUrl: 'https://api.vvhan.com/api/file.php',
                testMode: false
            }, options);
            ...
            var dom = {
                uploaderBox: $(this),
                submitButton: $('<button id="hansup" class="js-uploader__submit-button uploader__submit-button uploader__hide">' +
                ...
                selectButton: $('<input style="height: 0; width: 0;" id="fileinput' + index + '" type="file" class="js-uploader__file-input uploader__file-input">' +
                    ...
                    $.ajax({
                        ...
                        success: function (result) {
                            if (result.success == true) {
                                $("#okurl").html(result.downurl);
                            } else {
                                alert("文件最大支持50mb");
                            }
                        },
                        complete: function () {
                            $("#hansup").html('上传<i class="js-uploader__icon fa fa-upload uploader__icon"></i>');
                        }
                    });
                } else {
                    alert("每次最多上传一个文件");
