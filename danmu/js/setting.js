    ...
    if( danmuon > 0){
	var dp = new mbplayer({autoplay: autoplay,element: document.getElementById('player'),theme: color,logo: logo,video: {url: vodurl,pic: vodpic,type: 'auto',},danmaku: {id: dmid,api: dmapi,user: user}});
	}else{$('body').addClass("danmu-off");
	var dp = new mbplayer({autoplay: autoplay,element: document.getElementById('player'),theme: color,logo: logo,video: {url: vodurl,pic: vodpic,type: 'auto',},});}
    // 弹出框
    //function alert_back(text) {$(".alert_back").html(text).show();setTimeout(function () { $(".alert_back").fadeOut('slow'); },1200)}
    // 通用点击
	add('.mbplayer-list-icon',".mbplayer-danmu",'show');
    ...
    $(".mbplayer-showing").on("click", function () {dp.play();$(".vod-pic").remove();});
    //个性化弹幕框		
	$(".mbplayer-comment-setting-color input").on("click", function () {
    var textcolor = $(this).attr("value"); 
    setTimeout(function (){$('.bt_json-mbplayer-comment-input').css({"color":textcolor});}, 100);
    });
    $(".mbplayer-comment-setting-type input").on("click", function () {
    var texttype = $(this).attr("value"); 
    setTimeout(function (){$('.bt_json-mbplayer-comment-input').attr("dmtype",texttype);}, 100);
    });
    $("#dmset").on("click", function () {
    $(".mbplayer-comment-icon").trigger("click");
    $(".mbplayer-comment-setting-box").toggleClass("mbplayer-comment-setting-open") 
    $("#mbplayer").toggleClass("mbplayer-hide-controller") 
    });

    $(".bt_json-mbplayer-send-icon").on("click", function () {
    var inputtext = document.getElementById("dmtext");
    var sendtexts = inputtext.value;
    var sendtype =$('.bt_json-mbplayer-comment-input').attr("dmtype");
    var sendcolor = $('.mbplayer-comment-input').css("color"); 
    ...
    $(".bt_json-mbplayer-comment-input").val("");
    })
dp.danmaku.opacity(1);
    //弹幕列表获取
	
$(".mbplayer-list-icon,.bt_json-mbplayer-send-icon").on("click", function () {		   
	$(".list-show").empty();
	$.ajax({
    url:dmapi+"?id="+dmid,
    success:function (data) {
         ...
         $(".mbplayer-fulloff-icon").on("click", function () {dp.fullScreen.cancel();})
    //播放loading元素		
function video_play() {
...
function video_con_play() {
		...
		timer = setInterval(function(){
         ...
         var cplayer = `<div class="memory-play-wrap"><div class="memory-play"><span class="close">×</span><span>上次看到 </span><span>${ctime}</span><span class="play-jump">跳转播放</span></div></div>`
             $(".mbplayer-cplayer").append(cplayer);
